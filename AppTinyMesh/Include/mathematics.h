...
public:
    ...
    static constexpr double Cubic(double a, double b, double t);
...
constexpr double Math::Lerp(double a, double b, double t) {
    t = Clamp(t);
    return ((1 - t) * a) + (t * b);
}

constexpr double Math::Cubic(double a, double b, double t) {
    t = Clamp(t);
    double t2 = t * t;
    double t3 = t2 * t;
...
//! Cross product.
inline Vector operator/(const Vector &u, const Vector &v) {
    return Vector(u.c[1] * v.c[2] - u.c[2] * v.c[1], u.c[2] * v.c[0] - u.c[0] * v.c[2],
...
*/
inline Vector
Vector::Bilinear(const Vector &a00, const Vector &a10, const Vector &a11, const Vector &a01, double u, double v) {
...
public:
    ...
    static Matrix rotate(double angle, const Vector &up);

    static Matrix rotateAroundX(double angle);

    static Matrix rotateAroundY(double angle);

    static Matrix rotateAroundZ(double angle);
...
inline Matrix operator*(const Matrix &m, double v) {
    return v * m;
}
